...
> Port of the community skill `kubernetes-operator` (source `engineering/kubernetes-operator/skills/kubernetes-operator/SKILL.md`) from alirezarezvani/claude-skills for OpenCode. Name disambiguated as `cc-kubernetes-operator` (duplicate upstream name); body rebuilt with the upstream opencode transform.
