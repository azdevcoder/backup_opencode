...
> Port of the community skill `arquiteto-de-empresa` (source `c-level-advisor/arquiteto-de-empresa/skills/arquiteto-de-empresa/SKILL.md`) from alirezarezvani/claude-skills for OpenCode. Name disambiguated as `cc-arquiteto-de-empresa` (duplicate upstream name); body rebuilt with the upstream opencode transform.
