---
name: cc-engineering-kubernetes-operator
...
> Port of the community skill `kubernetes-operator` (source `engineering/skills/kubernetes-operator/SKILL.md`) from alirezarezvani/claude-skills for OpenCode. Name disambiguated as `cc-engineering-kubernetes-operator` (duplicate upstream name); body rebuilt with the upstream opencode transform.
