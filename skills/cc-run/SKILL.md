...
> Port of the community skill `run` (source `engineering/autoresearch-agent/skills/run/SKILL.md`) from alirezarezvani/claude-skills for OpenCode. Name disambiguated as `cc-run` (duplicate upstream name); body rebuilt with the upstream opencode transform.
