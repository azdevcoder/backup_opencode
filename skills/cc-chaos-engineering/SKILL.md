...
> Port of the community skill `chaos-engineering` (source `engineering/chaos-engineering/skills/chaos-engineering/SKILL.md`) from alirezarezvani/claude-skills for OpenCode. Name disambiguated as `cc-chaos-engineering` (duplicate upstream name); body rebuilt with the upstream opencode transform.
