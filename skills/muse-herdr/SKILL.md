...
> Port of the Muse Code skill `herdr` (extracted from Muse Code 1.3.0) for OpenCode. Original trigger semantics preserved; `bundled:<skill>` references were renamed to `muse-<skill>`. Muse-native tool calls (`read_skill`, `muse skills ...`, `muse exec/trace/export`, MSP session paths) map to OpenCode's `skill` tool and equivalent CLI steps here.

---

<!-- tbh: vendored from Herdr v0.9.0 `skills/herdr/SKILL.md` (https://github.com/herdrdev/herdr/blob/v0.9.0/skills/herdr/SKILL.md), Apache-2.0 licensed; re-vendor manually on Herdr upgrades (spec 27701 quickstart). -->
