...
> Port of the community skill `run` (source `engineering/agenthub/skills/run/SKILL.md`) from alirezarezvani/claude-skills for OpenCode. Name disambiguated as `cc-engineering-run` (duplicate upstream name); body rebuilt with the upstream opencode transform.
